...
Cache and Project Storage are flat, per-TB-per-month:

| Item | Rate |
|---|---|
| Cache | **$0.025 / TB / month** |
| Project Storage | **$0.015 / TB / month** |

Cache and Project Storage are billed every month regardless of whether you
run any jobs.
...
| Job stuck in PD with reason `QOSGrpBillingMinutes` | Your account's Weekly Cap on `billing` TRES was reached | `sbalance --week` to confirm; switch partitions / wait for Sunday 00:00 (cluster time), when the weekly window resets / ask the PI to raise the cap |
...
| Storage charge appears on bill with 0 hours run | Cache and Project Storage are charged monthly regardless of compute activity | This is by design; see the storage billing note above |
